...
2. **视觉回归的可用性**：已经解决——参考图一直都在画文本与填色，此前"渲染环境不画文本"的判断是错的（见下文《截图内容核查》）。剩下的工作是把逐组件的关键颜色/几何断言从目前 7 项扩展到更多组件。
...
同时新增 `UPScreenshotContentTest`（7 项 JVM 单测，走 `javax.imageio` 读回已提交的 PNG），
...
- 全部 7 张新参考图都不是空白（非白像素 > 5000、配色数 ≥ 20）
