...
2. **视觉回归的可用性**：已经解决——参考图一直都在画文本与填色，此前"渲染环境不画文本"的判断是错的（见下文《截图内容核查》）。目前 10 项像素级断言覆盖了本轮改动的 7 张参考图，剩下的工作是把同样的手法推广到其余 21 张。
...
同时新增 `UPScreenshotContentTest`（**10 项 JVM 单测**，走 `javax.imageio` 读回已提交的 PNG），
